...
void LightWeightEnemy::attack(Cell* tagret_cell)
{
	auto building = tagret_cell->get_building();
	Castle* castle = static_cast<Castle*>(building);
	if (castle->alive()) {
		castle->get_damage(_damage);
	}
}

void LightWeightEnemy::go()
{
...
Cell* LightWeightEnemy::find_target()
{
...
LightWeightEnemy::LightWeightEnemy(COORD coordinates, size_t HP, size_t Max_HP, size_t regeneration, size_t damage, Map* map)
	: Enemy(coordinates, 'l', HP, Max_HP, regeneration, damage, map)
{}