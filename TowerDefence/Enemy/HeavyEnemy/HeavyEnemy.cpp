...
void HeavyEnemy::attack(Cell* tagret_cell)
{
	auto building = tagret_cell->get_building();

	if (dynamic_cast<Wall*>(building)) {
		static_cast<Wall*>(building)->get_damage(_damage);
		return;
	}
...
void HeavyEnemy::go()
{
	...
	while (!open.empty() && !find_flag)
	{
		...
		if (_map->in_map(neighbour_coords.X, neighbour_coords.Y)
			...
			&& (_map->get(neighbour_coords.X, neighbour_coords.Y)->get_type() == Cell_types::_Land
				|| _map->get(neighbour_coords.X, neighbour_coords.Y)->get_type() == Cell_types::_Wall
				|| _map->get(neighbour_coords.X, neighbour_coords.Y)->get_type() == Cell_types::_Castle)
	...
	for (size_t x = _coordinates.X - _radius; x < _coordinates.X + _radius; ++x)
	{
		for (size_t y = _coordinates.Y - _radius; y < _coordinates.Y + _radius; ++y)
		{
			if (_map->in_map(x, y)) {
				...
				if (building && (building->get_type() == Cell_types::_Castle || building->get_type() == Cell_types::_Wall)) {
					f_building = building;
				}
			}
		}
...
HeavyEnemy::HeavyEnemy(COORD coordinates, size_t HP, size_t Max_HP, size_t regeneration, size_t damage, Map* map) :
	Enemy(coordinates, 'h', HP, Max_HP, regeneration, damage, map)
{}